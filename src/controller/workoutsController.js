 ...
 * This is an object that represents the HTTP request and has
...
function getWorkoutById(req, res) {
  const workout = workoutsService.getWorkoutById(parseInt(req.params.id, 10));
  if (workout) {
    res.json(workout);
  } else {
...
function updateWorkout(req, res) {
  const workout = workoutsService.updateWorkout(parseInt(req.params.id, 10), req.body);
  if (workout) {
    res.json(workout);
  } else {
    res.status(404).send('Workout not found');
...
function deleteWorkout(req, res) {
  const workout = workoutsService.deleteWorkout(Number(req.params.id));
  if (workout) {
    res.json(workout);
  } else {
    res.status(404).send('Workout not found');
